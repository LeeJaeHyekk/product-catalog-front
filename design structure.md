...
* ✅ `select` → `useMemo` 전환 가능 (유연성)
* ✅ Zod 검증 → 경량 검증 전환 가능 (성능)
* ✅ 일반 Grid → Virtualization 전환 가능 (대용량)
* ✅ 단일 함수 → 단계별 함수 분리 가능 (복잡도 증가)

### 16.6 설계의 한계 및 개선 방향

**현재 한계:**

* API에 고유 ID가 없어 key 전략이 임시 방편
* 실서비스 환경에서는 서버에서 고유 ID 제공 필요

**개선 방향:**

* 서버 API 스펙 개선 (고유 ID 추가)
* 점진적 기능 확장 (필터, 정렬, 검색)
* 성능 모니터링 및 최적화

### 16.7 최종 평가

이 설계는 **실무 환경 및 과제 제출 환경 모두에 적합**하며,
다음과 같은 이유로 확장 가능한 구조를 제공합니다:

* ✅ 단위 테스트 가능한 비즈니스 로직
* ✅ 명확한 책임 분리로 유지보수 용이
* ✅ TanStack Query로 안정적인 서버 상태 관리
* ✅ 확장 가능한 함수형 구조
* ✅ Core Web Vitals 고려한 UX 최적화
* ✅ 실무 현실성을 고려한 유연한 설계
* ✅ 대용량 데이터 대응 전략 포함
* ✅ **성능 최적화 전략 명시** (렌더링 최소화, 메모이제이션, 정렬 책임 분리)
* ✅ **체감 성능 개선** (Skeleton UX, Cache 전략)
* ✅ 솔직한 한계점 인정 및 개선 방향 제시