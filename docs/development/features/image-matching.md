...
상품명과 이미지 파일명을 자동으로 매칭하여 적절한 이미지를 표시하는 기능입니다. 3단계 검증 시스템을 통해 정확한 매칭을 시도합니다.
...
자세한 내용은 [형태소 분석 라이브러리 가이드](./morphology-library-guide.md)를 참고하세요.
