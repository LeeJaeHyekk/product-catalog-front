...
| 항목 | 상태 | 비고 |
|------|------|------|
| `package.json` 설정 | ✅ | `"type": "module"` 설정됨 |
| `tsconfig.json` 설정 | ✅ | `"module": "ESNext"` 설정됨 |
| Import 문법 | ✅ | 모든 파일에서 `import` 사용 |
| Export 문법 | ✅ | 모든 파일에서 `export` 사용 |
| CommonJS 사용 | ✅ | `require`/`module.exports` 사용 없음 |
